...
async function countOrders(start: string, end: string, filter?: CountFilter) {
...
export async function GET(request: Request) {
  try {
    ...
    const [
      ...
      countOrders(start, endExclusive),
      countOrders(start, endExclusive, { column: "order_type", value: DINE_IN_VALUES }),
      countOrders(start, endExclusive, { column: "order_type", value: "Takeaway" }),
      countOrders(start, endExclusive, { column: "status", value: "Pending" }),
      countOrders(start, endExclusive, { column: "status", value: COMPLETED_VALUES }),
      supabase
        .from("orders")
    ...
    const rangeRows = (rangeRowsResult.data || []) as OrderAnalyticsRow[];
    const trendRows = (trendRowsResult.data || []) as Pick<OrderAnalyticsRow, "total_amount" | "created_at">[];
